...
Responsibility List:
  ...
  Kylee Hager: Mainly library and documentation, helped with fileclient and readMe, collected files together and joined makefiles.
...
Assumptions:
	It is assumed that the user knows the host and port numbers for the server they want to connect to when using the client. The user should also be able to realize that entering “help” will give them a list of commands they can perform. Capitalization should also be strictly followed. Examples of proper capitalization can be found in the "help" command.

	For using put, assume the local path should be a relative path (format: "./path/file") and the remote path should be a root path of the simulated client directory (format: /clientHome/file).
	Get -R must be used on a directory.
